...
function M.warp_to(surface, planet, extra_data)
    local should_recall = storage.stabilizer.settings.recall == true
    local cost = (extra_data and extra_data.cost) or (1 + (should_recall and 0.1 or 0))
    if storage.stabilizer.charge.current < cost then return end
    ...
    storage.stabilizer.warping = { to = planet, warp_tick = game.tick + 90, finished_tick = game.tick + 180, cost = cost, recall = should_recall }
    ...
    for p, _ in pairs(config.planets) do
        if p == planet or (extra_data and extra_data.fixed_followup and p ~= extra_data.fixed_followup) then
